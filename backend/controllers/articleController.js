      ...
      .then((result) => {
        if (result == null || result.length == 0) {
          throw new HTTPException(
            "Can't find any article by your id",
            HTTP.NOT_FOUND
          );
        }
...
const addArticle = async (req, res) => {
  try {
    if (!checkRights(req.userData.userID, 5)) {
      throw new HTTPException(
        'No admin rights for add new article',
        HTTP.FORBIDDEN
      );
    }

    const authorInfo = await User.findById({ _id: req.userData.userID }).then(
      (result) => {
        if (result.length == 0) {
          throw new HTTPException(
            'There are no user by that id',
            HTTP.BAD_REQUEST
          );
        }
        return result;
    ...
    if (!subCategory) {
      throw new HTTPException(
        'Sub category id does not exist',
        HTTP.BAD_REQUEST
      );
    }
    ...
    const categoryInfo = await Category.findById({ _id: category }).catch(
      (error) => {
        throw new HTTPException('Wrong category id', HTTP.NOT_FOUND);
      }
    );

    const subCategoryInfo = await SubCategory.findById({
      _id: subCategory,
    }).catch((error) => {
    ...
    fileType.fromBuffer(Buffer.from(base64image, 'base64')).then((result) => {
    ...
    for (let i in slider) {
      ...
      const base64slider = slider[i].replace(
        /^data:([A-Za-z-+/]+);base64,/,
        ''
      );

      fileType
        .fromBuffer(Buffer.from(base64slider, 'base64'))
        .then((result) => {
          if (!result.mime.includes('image'))
            throw new HTTPException('File is no image', HTTP.FORBIDDEN);
        });
    ...
    return res
      .status(HTTP.OK)
      .json({ message: 'Article created successfuly', article });
  } catch (exception) {
    if (!(exception instanceof HTTPException)) {
      exception.statusCode = HTTP.INTERNAL_SERVER_ERROR;
  ...
  try {
    ...
    if (image) {
      if (image != '' || image != [] || image != undefined) {
        ...
        fileType
          .fromBuffer(Buffer.from(base64image, 'base64'))
          .catch((exception) => {
            if (exception)
    ...
    if (newSlider) {
      if (newSlider != [] || newSlider != '' || newSlider != undefined) {
        console.log('new slider', newSlider);
        for (let i of newSlider) {
          ...
          fs.writeFileSync(sliderImagePath, base64slider, {
            encoding: 'base64',
          });
    ...
    if (deleteSlider) {
      if (
        deleteSlider != [] ||
        deleteSlider != '' ||
        deleteSlider != undefined
      ) {
        console.log('delete slider', deleteSlider);
...
const deleteArticle = async (req, res) => {
  try {
    ...
    const slides = await Slider.find({ article: req.params.id })
      .catch((exception) => {
        if (exception) {
          throw new HTTPException('Wrong id', HTTP.BAD_REQUEST);
        }
